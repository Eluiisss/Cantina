                    ...
                    <div class="relative">
                        <select name="orderStatus"
                            ...
                            @foreach(trans('orders.manage.filters.order') as $value => $text)
                                <option value="{{ $value }}" {{ request('orderStatus') === $value ? 'selected' : '' }} >{{ $text }}</option>
                            @endforeach
                        </select>
                        <div
                ...
                <div class="flex flex-row mb-1 sm:mb-0">
                    <div class="relative">
                        <select name="paymentStatus"
                                class=" main-color-blue-bg main-color-yellow-text appearance-none h-full rounded-r border-t sm:rounded-r-none sm:border-r-0 border-r border-b block appearance-none w-full bg-white border-gray-400 text-gray-700 py-2 px-4 pr-8 leading-tight focus:outline-none focus:border-l focus:border-r">
                            @foreach(trans('orders.manage.filters.payment') as $value => $text)
                                <option value="{{ $value }}" {{ request('paymentStatus') === $value ? 'selected' : '' }} >{{ $text }}</option>
                            @endforeach
                        </select>
                        <div
                            class="pointer-events-none absolute inset-y-0 right-0 flex items-center px-2 text-gray-700">
