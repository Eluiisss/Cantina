        ...
        <form method="POST" action="{{ route('login') }}">
            ...
            <div class="block mt-4">
                <label for="remember_me" class="inline-flex items-center">
                    ...
                    <span class="ml-2 text-sm main-color-blue-text">{{ __('Recuérdame') }}</span>
                </label>
            </div>

            <div class="flex items-center justify-end mt-4">
                @if (Route::has('password.request'))
                    <a class="underline text-sm main-color-blue-text" href="{{ route('password.request') }}">
                        {{ __('¿Has olvidado tu contraseña?') }}
                    </a>
                @endif

                <x-button class="ml-3 main-color-blue-bg main-color-yellow-text font-bold">
                    {{ __('Iniciar sesión') }}
                </x-button>
            </div>
        </form>
